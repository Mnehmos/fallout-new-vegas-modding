"""Rebuild ArchivistsCache.esp with all weapons renamed to avoid conflicts."""
import sys, struct
...
esp = ESPBuilder(author='Mnehmos', description='The Archivists Cache')
esp.add_master('FalloutNV.esm')
esp.add_master('MnehmosMojave.esp')
...
# Helper: manual clone with DNAM proj count patch
...
# ═══ EUREKA — Quad laser rifle (renamed from Bright Idea) ═══
...
# ═══ CATHARSIS — Quad multiplas (renamed from Overexposure) ═══
...
# ═══ CONTAINER ═══
cs = find_weap.__self__ if False else None  # dummy
...
items = [
    ...
    (0x0008ED78, 60),   # 20ga
    (0x00121158, 60),   # .357
    (0x00078CC2, 200),  # Microfusion
    (0x00078740, 200),  # Energy cells
    (0x00078744, 15),   # Stimpaks
    (0x00015197, 10),   # Purified Water
]
for fid, cnt in items:
    cont_rec.add_bytes('CNTO', struct.pack('<Ii', fid, cnt))
esp.add_record('CONT', cont_rec)

out = esp.save('H:/01_Games/Steam/steamapps/common/Fallout New Vegas/Data/ArchivistsCache.esp')
print(f'\nSaved: {out}')
print(f'Trunk: {len(items)} items, no name conflicts')
